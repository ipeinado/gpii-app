/**
 * The QSS Excel Simplification widget
 *
...
(function (fluid) {
    ...
    /**
     * Represents the QSS MS Excel simplification widget.
     */
    fluid.defaults("gpii.qssWidget.excel", {
        ...
        selectors: {
            heightListenerContainer: ".flc-qssExcelWidget-controls",
            menuControlsWrapper: ".flc-qssExcelWidget-controlsWrapper",
            menuControls: ".flc-qssExcelWidget-controls",
        ...
        components: {
            repeater: {
                ...
                container: "{excel}.dom.menuControls",
                options: {
                    model: {
                        disabled: "{excel}.model.disabled",
                        value: "{excel}.model.setting.value",
                        styles: "{excel}.model.setting.styles"
                    },
                    modelRelay: {
                        items: {
                            target: "items",
                            singleTransform: {
                                ...
                                func: "gpii.qssWidget.excel.getRepeaterItems",
                                args: [
                                    "{excel}.model.setting"
                                ]
                            }
                    ...
                    handlerType: "gpii.qssWidget.excel.presenter",
                    markup: null,
                    styles: {
                        disabled: "disabled"
                    },
        ...
        listeners: {
            "onCreate.visibilityChange": {
                funcName: "gpii.qssWidget.excel.addVisibilityChangeListener",
                args: ["{closeTimer}"]
            },
            "onDestroy.visibilityChange": {
                funcName: "gpii.qssWidget.excel.removeVisibilityChangeListener"
            }
        },
        invokers: {
            calculateHeight: {
            ...
            close: {
                funcName: "gpii.qssWidget.excel.close",
                args: [
                    "{that}",
                    "{closeTimer}",
     ...
     */
    gpii.qssWidget.excel.getCommand = function (states, availableCommands) {
     ...
     */
    gpii.qssWidget.excel.fillStates = function (keys, value) {
        var filledStates = {};
     ...
     */
    gpii.qssWidget.excel.changeState = function (states, key, value) {
    ...
    /**
     * Pre-loads the data in the excel.model.states array
     * @param {Component} that - The `gpii.qssWidget.excel.presenter` instance.
     * @param {Component} excel - The `gpii.qssWidget.excel` instance.
     * @param {String} ribbonState - The initial state of ribbons
     */
    gpii.qssWidget.excel.loadState = function (that, excel) {
        // pre-fills the states of all available schema keys
        var currentValue = excel.model.setting.value,
            commands = excel.model.availableCommands,
            schemaKeys = excel.model.setting.schema.keys;

        if (currentValue === commands.allTrueCommand) {
            // pre-fill the states with true
            excel.applier.change("states", gpii.qssWidget.excel.fillStates(schemaKeys, true), false, "fromWidget");
        } else if (currentValue === commands.allFalseCommand) {
            // pre-fill the states with false
            excel.applier.change("states", gpii.qssWidget.excel.fillStates(schemaKeys, false), false, "fromWidget");
        } else {
            // pre-fill the states with false first
            var newStates = gpii.qssWidget.excel.fillStates(schemaKeys, false);
            // now change only the selected key using the current value as a key
            excel.applier.change("states", gpii.qssWidget.excel.changeState(newStates, currentValue, true), false, "fromWidget");
        }

        // checks the required checkboxes
        gpii.qssWidget.excel.applyCheckmarks(that, excel.model.states);
    };
     ...
     * @param {Object.<String, Boolean>} states - simple true/false object with the current states
     */
    gpii.qssWidget.excel.applyCheckmarks = function (that, states) {
    ...
    /**
     * Updates the value in the model of the widget's repeater based on the excel model's states
     * @param {Component} that - The `gpii.qssWidget.excel.presenter` instance.
     ...
     */
    gpii.qssWidget.excel.updateValue = function (that, key, container, excel, repeater) {
        // toggle the current state
        var newStates = gpii.qssWidget.excel.changeState(excel.model.states, key, !excel.model.states[key]);

        // apply the change to the state
        excel.applier.change("states", newStates, false, "fromWidget");
        ...
        // applying the value
        var commandToUse = gpii.qssWidget.excel.getCommand(newStates, excel.model.availableCommands);
     ...
     * @param {Component} that - The `gpii.qssWidget.excel` instance.
     ...
     */
    gpii.qssWidget.excel.close = function (that, closeTimer, keyboardEvent) {
     ...
     * the widget with a delay.
     */
    gpii.qssWidget.excel.addVisibilityChangeListener = function (closeTimer) {
        $(document).on("visibilityChange.qssExcelWidget", function () {
     ...
     * destroyed.
     */
    gpii.qssWidget.excel.removeVisibilityChangeListener = function () {
        $(document).off("visibilityChange.qssExcelWidget");
    };
     ...
     */
    gpii.qssWidget.excel.getRepeaterItems = function (setting) {
     ...
     */
    fluid.defaults("gpii.qssWidget.excel.presenter", {
        ...
        listeners: {
            "onCreate.applyStyles": {
                funcName: "gpii.qssWidget.excel.presenter.applyStyles",
                args: ["{that}", "{that}.container", "{repeater}.model.styles"]
            },
            "onCreate.loadState": {
                funcName: "gpii.qssWidget.excel.loadState",
                args: [
                    "{that}",
                    "{excel}"
                ]
            ...
            onItemFocus: {
                funcName: "gpii.qssWidget.excel.presenter.focusItem",
                args: [
                    "{that}",
                    "{focusManager}",
                    "{that}.container",
        ...
        invokers: {
            activate: {
                funcName: "gpii.qssWidget.excel.updateValue",
                args: [
                    "{that}",
                    "{that}.model.item.key",
                    "{that}.container",
     ...
     */
    gpii.qssWidget.excel.presenter.focusItem = function (that, focusManager, container, index) {
     ...
     */
    gpii.qssWidget.excel.presenter.applyStyles = function (that, container, styles) {
